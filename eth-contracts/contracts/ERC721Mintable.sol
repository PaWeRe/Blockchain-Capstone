...
contract Ownable {
    ...
    require(msg.sender != address(0), 'Sender must dispose of a valid address!');
...
contract Pausable is Ownable {
    ...
    function pause(address addressAccount) public onlyOwner whenNotPaused{
            _paused = true;
            emit Paused(addressAccount);

    } 

    function unpause(address addressAccount) public onlyOwner paused{
        _paused = false;
        emit Unpaused(addressAccount);
    }
    ...
    event Paused(address addressAccount);
    event Unpaused(address addressAccount);
}

/*That is what ERC165 is for — a standard to publish and detect what interfaces a smart contract implements.
Medium article: https://medium.com/coinmonks/ethereum-standard-erc165-explained-63b54ca0d273#:~:text=In%20smart%20contract%20development%2C%20a,call%20functions%20from%20another%20contract.*/
contract ERC165 {
...
contract ERC721 is Pausable, ERC165 {
    ...
    // see: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/56de324afea13c4649b00ca8c3a3e3535d532bd4/contracts/utils/Counters.sol
    ...
    constructor () public {
        // register the supported interfaces to conform to ERC721 via ERC165 (apperently no point operator necessary)
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, 'Account is already the owner of the token!');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender) == true, 'Sender must be the owner of the contract or it must be approved for all!');
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to != address(0),'Inputted address for minting is invalid!');
        ...
        emit Transfer(address(0), to, tokenId); //Tokens are created (minted), therefore sender = address(0)!
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(to != address(0), 'Token must be transferred to valid address!');

        // TODO: clear approval (:= set previous owner of TokenId)
...
contract CustomERC721Token is ERC721Metadata {
    ...
    function mint(address to, uint256 tokenId) public returns(bool){
        require(msg.sender == getOwner(), "Only contract owner can call the mint()-function!");
